...
const macarrones = unProducto('Macarrones', { marca: 'Gallo', fibra: 3 })

const registro = (productoId: string, extra: Partial<Registro> = {}): Registro =>
  unRegistro(productoId, {
    fecha: AYER,
...
describe('duplicarDia', () => {
  it('copia todos los registros al dia destino', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id, { momento: 'desayuno' }))
    await db.diario.add(registro(id, { momento: 'cena', cantidad: 200 }))

    expect(await duplicarDia(AYER, HOY)).toBe(2)
    const deHoy = await db.diario.where('fecha').equals(HOY).toArray()
  ...
  it('no toca el dia de origen', async () => {
    ...
    expect(await db.diario.where('fecha').equals(AYER).count()).toBe(1)
  })

  it('respeta las cantidades', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id, { cantidad: 250 }))
    await duplicarDia(AYER, HOY)
    const r = (await db.diario.where('fecha').equals(HOY).toArray())[0]
  ...
  it('recalcula con el producto de hoy, no con los macros congelados de ayer', async () => {
    ...
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id))
    await db.alimentos.update(id, { kcal: 300 })

    await duplicarDia(AYER, HOY)
    const ayer = (await db.diario.where('fecha').equals(AYER).toArray())[0]
    const hoy = (await db.diario.where('fecha').equals(HOY).toArray())[0]
  ...
  it('actualiza el nombre si el producto se renombro', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id))
    await db.alimentos.update(id, { nombre: 'Macarrones integrales', marca: undefined })
  ...
  it('si el producto ya no existe copia los valores congelados', async () => {
    ...
    await db.diario.add(registro('fantasma'))
  ...
  it('genera ids nuevos en vez de pisar los de origen', async () => {
    ...
    await db.diario.add(registro(id))
    await duplicarDia(AYER, HOY)
    const todos = await db.diario.toArray()
  ...
  it('conserva el orden dentro del dia', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id, { nombreProducto: 'A', creadoEn: 1 }))
    await db.diario.add(registro(id, { nombreProducto: 'B', creadoEn: 2 }))
    await db.diario.add(registro(id, { nombreProducto: 'C', creadoEn: 3 }))

    await duplicarDia(AYER, HOY)
    const hoy = (await db.diario.where('fecha').equals(HOY).toArray()).sort(
  ...
  it('duplicar sobre el mismo dia no duplica nada', async () => {
    ...
    expect(await db.diario.count()).toBe(1)
  })

  it('se suma a lo que ya hubiese en el destino', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id))
    await db.diario.add(registro(id, { fecha: HOY, momento: 'desayuno' }))

    await duplicarDia(AYER, HOY)
    expect(await db.diario.where('fecha').equals(HOY).count()).toBe(2)
  })
})
...
describe('diaAnteriorConDatos', () => {
  it('encuentra el dia con registros mas reciente', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id, { fecha: '2026-09-01' }))
    await db.diario.add(registro(id, { fecha: '2026-09-05' }))
    await db.diario.add(registro(id, { fecha: '2026-09-03' }))
  ...
  it('no mira el dia actual ni el futuro', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id, { fecha: HOY }))
    await db.diario.add(registro(id, { fecha: '2026-12-25' }))
  ...
  it('cruza el cambio de año', async () => {
    const id = await db.alimentos.add(macarrones)
    await db.diario.add(registro(id, { fecha: '2025-12-31' }))
